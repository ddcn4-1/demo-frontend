...
export function Breadcrumb() {
  ...
  const getBreadcrumbs = () => {
    ...
    if (path === '/' || path === '/performances') {
      ...
      if (tab === 'history') {
        breadcrumbs.push({ label: 'My Bookings', path: '/dashboard?tab=history' });
      } else {
        breadcrumbs.push({ label: 'Performances', path: '/dashboard?tab=performances' });
      }
      ...
      if (tab) {
        ...
        if (tabLabels[tab]) {
          breadcrumbs.push({ label: tabLabels[tab], path: `/admin?tab=${tab}` });
        }
      }
      ...
      if (segments.length >= 3) {
        breadcrumbs.push({ label: 'Performance Details', path: path });
